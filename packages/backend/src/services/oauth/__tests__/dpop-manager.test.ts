...
describe('DPoP Manager', () => {
  ...
  describe('DPoP Proof Creation', () => {
    ...
    it('should handle uninitialized manager', async () => {
      ...
      await expect(
        uninitializedManager.createProof('GET', 'https://example.com')
      ).rejects.toThrow('DPoP key pair not initialized');
    });
  });
  ...
  describe('AT Protocol Integration', () => {
    ...
    it('should create AT Protocol API call proof', async () => {
      const apiEndpoint = 'https://bsky.social/xrpc/com.atproto.repo.createRecord?collection=app.bsky.feed.post';
      
      const proof = await atProtocolDPoP.createATProofForAPICall(
        dPopManager,
        'POST',
        apiEndpoint
      );
